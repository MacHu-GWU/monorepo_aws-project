...
from ..schema import (
    ...
    dynamodb_json_to_simple_schema,
)
...
class Case:
    ...
    def test_polars_type_to_simple_type(self):
        df = pl.DataFrame([self.item])
        simple_schema = dict()
        for k, v in df.schema.items():
            simple_type = polars_type_to_simple_type(v)
            simple_schema[k] = simple_type
        # for k in self.simple_schema: # for debug only
        #     print("---")
        #     print(simple_schema[k])
        #     print(self.simple_schema[k])
        assert simple_schema == self.simple_schema

    def test_dynamodb_json_to_simple_schema(self):
        simple_schema = dynamodb_json_to_simple_schema(self.json)
        assert simple_schema == self.simple_schema
