...
First, let's create a new project code skeleton using a sample project. It will prompt you to enter your project name and some configuration::
    ...
    # assume that you are currently at the ``./projects/simple_lbd_container-project`` directory.
...
Every project requires a collection of AWS resources for CI/CD. To provision these resources, we need to run a script called ``bootstrap``. Please refer to the ``./projects/simple_lbd_container-project/bootstrap/README.rst`` file for more details. You can use the following command to provision the CI/CD AWS resource::
...
------------------------------------------------------------------------------
In this step, we focus on the core application code logic of the project. From this point, I assume that you are currently at the ``./projects/simple_lbd_container-project`` (``./projects/${du_name}-project``) directory.

1. Create a feature branch: ``simple_lbd_container/feature/${description}`` (``${du_name}/feature/${description}``) and start development.
...
10. Now you can merge the ``simple_lbd_container/feature/${description}`` to ``main``.
...
1. Create a layer branch ``simple_lbd_container/layer/${description}`` (``${du_name}/layer/${description}``).
...
4. Once you see a new layer is published, you can merge the ``simple_lbd_container/layer/${description}`` to ``main``.
...
1. Create a release branch ``simple_lbd_container/release`` (``${du_name}/release``). Don't do any application code change in this branch. If you have to, please roll back to the previous step to ensure that your change doesn't break the test.
...
- Create a cleanup branch: ``simple_lbd_container/cleanup`` (``${du_name}/cleanup``). Don't do any application code change in this branch.
