...
First, let's create a new project code skeleton using a sample project. It will prompt you to enter your project name and some configuration::
    ...
    # assume that you are currently at the ``./projects/aws_idp_data_store-project`` directory.
    python ../../cookiecutter/aws_idp_data_store.py
...
------------------------------------------------------------------------------
In this step, we focus on the core application code logic of the project. From this point, I assume that you are currently at the ``./projects/aws_idp_data_store-project`` (``./projects/${du_name}-project``) directory.

1. Create a feature branch: ``aws_idp_data_store/feature/${description}`` (``${du_name}/feature/${description}``) and start development.
...
10. Now you can merge the ``aws_idp_data_store/feature/${description}`` to ``main``.
...
1. Create a layer branch ``aws_idp_data_store/layer/${description}`` (``${du_name}/layer/${description}``).
...
4. Once you see a new layer is published, you can merge the ``aws_idp_data_store/layer/${description}`` to ``main``.
...
1. Create a lambda branch ``aws_idp_data_store/lambda/${description}`` (``${du_name}/lambda/${description}``).
...
6. Once you see the app is deployed to ``sandbox`` and the integration test is passed, you can merge the ``aws_idp_data_store/lambda/${description}`` to ``main``.
...
1. Create a release branch ``aws_idp_data_store/release`` (``${du_name}/release``). Don't do any application code change in this branch. If you have to, please roll back to the previous step to ensure that your change doesn't break the test.
...
- Create a cleanup branch: ``aws_idp_data_store/cleanup`` (``${du_name}/cleanup``). Don't do any application code change in this branch.
