...
The ``cdk`` folder
------------------------------------------------------------------------------
The ``cdk`` folder is the CDK official convention where you should store your IAC declaration code and where you should run the ``cdk deploy`` command. However, for enterprise project, we should break down complicated logics into modules for better maintainability. The ``cdk/app.py`` script just import the declaration code from the ``aws_idp_data_store/iac/`` Python modules.


The ``aws_idp_data_store/iac/`` Python Modules
------------------------------------------------------------------------------
``aws_idp_data_store/iac/`` is the root folder of the infrastructure as code source code. Below are the list of important files related to Infrastructure as Code::

    aws_idp_data_store/iac # the root folder of the infrastructure as code source code
    aws_idp_data_store/iac/define/ # CDK stack declaration code
    aws_idp_data_store/iac/define/main.py # centralized CDK stack object, AWS resources are break down into sub-modules
    aws_idp_data_store/iac/define/iam.py # IAM related AWS resources
    aws_idp_data_store/iac/define/lbd.py # Lambda function related AWS resources
    aws_idp_data_store/iac/exports.py # CDK stack output exports for other projects to use
...
------------------------------------------------------------------------------
1. Working on ``aws_idp_data_store/iac/`` module to declare your CDK stack.
